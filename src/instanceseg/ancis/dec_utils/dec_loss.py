...
class DecLoss(nn.Module):
    ...
    def forward(self, predictions, gt_bboxes, gt_labels):
        ...
        # confidence loss
        # negative hard mining (as proposed in original SSD)
        pos_mask = pos_mask.squeeze(2)
        p_conf_batch = p_conf.view(-1, self.num_classes)
        temp = self.log_sum_exp(p_conf_batch)
        temp = temp - p_conf_batch.gather(dim=1, index=t_conf.view(-1,1))
        temp = temp.view(batch_size, -1) # size [batch_size, n_anchors]
        temp[pos_mask] = 0.
        _, temp_idx = temp.sort(1, descending=True)
        _, idx_rank = temp_idx.sort(1)
        num_neg = torch.clamp(self.neg_pos_ratio * num_pos,
                              max=pos_mask.size(1)-1).squeeze(2)
        neg_mask = idx_rank < num_neg.expand_as(idx_rank)

        # conf loss calc
        pos_conf_mask = pos_mask.unsqueeze(2).expand_as(p_conf)
        neg_conf_mask = neg_mask.unsqueeze(2).expand_as(p_conf)
        input = p_conf[(pos_conf_mask + neg_conf_mask).gt(0)].view(-1, self.num_classes)
        target = t_conf[(pos_mask + neg_mask).gt(0)].squeeze(1)
        loss_conf = F.cross_entropy(input=input, target=target, reduction='sum')
        N = num_pos.data.sum()
        if N == 0:
            N = 1.
        N = N.float()
